...
void SemaphoreTest(int iTaskId)
{
    ...
    iRet = Ql_OS_TakeSemaphore(s_iSemaphoreId, 0xFFFFFFFF);//TRUE or FLASE indicate the task should wait infinitely or return immediately.
    APP_DEBUG("\r\n<--=========Ql_osTakeSemaphore(%d, TRUE) = %d-->", s_iSemaphoreId, iRet);   //Flag Sentence

    Ql_OS_TakeMutex(s_iSemMutex,0xFFFFFFFF);
    s_iTestSemNum--;
...
void SemaphoreTest1(int iTaskId)
{
    ...
    while(1)
    {
        iRet = Ql_OS_TakeSemaphore(s_iSemaphoreId, 0);
        if(OS_SUCCESS == iRet)
        {
    ...
    switch (msg)
    {
        case EVENT_UART_READY_TO_READ:
        {
            ...
            else if(NULL != (p = Ql_strstr(pData, "TestSemaphore")))
            {
                ...
                s_iSemaphoreId = Ql_OS_CreateSemaphore(s_iTestSemNum,s_iTestSemNum);
                APP_DEBUG("\r\n<--Ql_osCreateSemaphore(\"MySemaphore\", %d)=%08x -->\r\n", s_iTestSemNum, s_iSemaphoreId);

                s_iSemMutex= Ql_OS_CreateMutex();
