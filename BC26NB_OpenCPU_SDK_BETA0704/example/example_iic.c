...
u8 I2C_SendBuf[I2C_SENDBUF_LENGTH] ={0x01,0x16,0x17,0x18,0x19,0x20}; //register addr :0x01
    ...
    switch (msg)
    {
        case EVENT_UART_READY_TO_READ:
        {
            ...
            if(p)
            {
                ...
                {                    
                    ret = Ql_IIC_Init(0,PINNAME_CTS_AUX,PINNAME_RTS_AUX,0);
                    if(ret < 0)
                    {
	     ...
	     //command-->IIC config,  IIC controller interface
            ...
            if(0 == ret)
            {
                ret = Ql_IIC_Write(0,I2C_SlaveAddr,(u8*)I2C_SendBuf,6);
                if(ret < 0)
                {
            ...
            if(0 == ret)
            {
                ret = Ql_IIC_Write(1,I2C_SlaveAddr,(u8*)I2C_SendBuf,6);
                if(ret < 0)
                {
            ...
            if(0 == ret)
            {
                ...
                ret = Ql_IIC_Read(0, I2C_SlaveAddr, I2C_RecvBuf, 4);     //read data
                if(ret < 0)
                {
            ...
            if(0 == ret)
            {
                ...
                ret = Ql_IIC_Read(1, I2C_SlaveAddr, I2C_RecvBuf, 4);//read data
                if(ret < 0)
                {
            ...
            if(0 == ret)
            {
                ret =  Ql_IIC_Write_Read(0,I2C_SlaveAddr,(u8*)I2C_SendBuf,1,(u8*)I2C_RecvBuf,4);
                if(ret < 0)
                {
            ...
            if(0 == ret)
            {
                ret = Ql_IIC_Write_Read(1, I2C_SlaveAddr, (u8*)I2C_SendBuf, 1,I2C_RecvBuf, 4);
                if(ret < 0)
                {
