...
#define RIL_LwM2M_DEBUG_ENABLE 1
...
s32 RIL_QLwM2M_Send(ST_Lwm2m_Send_Param_t *lwm2m_send_param_t)
{
	...
	if(LWM2M_SEND_MODE_NON == lwm2m_send_param_t->lwm2m_send_mode)
	{
	  ...
	  lwm2m_send_param_t->ins_id,lwm2m_send_param_t->res_num,lwm2m_send_param_t->send_length,lwm2m_send_param_t->buffer,"0x0000");
      ret = Ql_RIL_SendATCmd(strAT,Ql_strlen(strAT),ATResponse_Handler,NULL,0);
	}
	else if(LWM2M_SEND_MODE_CON == lwm2m_send_param_t->lwm2m_send_mode)
	{
	  Ql_sprintf(strAT, "AT+QLWDATASEND=%d,%d,%d,%d,%s,%s\n",lwm2m_send_param_t->obj_id,\
	  lwm2m_send_param_t->ins_id,lwm2m_send_param_t->res_num,lwm2m_send_param_t->send_length,lwm2m_send_param_t->buffer,"0x0100");
...
s32 RIL_QLwM2M_RD(u32 rd_length,u32* actual_length,u32* remain_length,u8* data_buffer)
{
	...
	Lwm2m_recv_param.actual_length = 0;
	Lwm2m_recv_param.buffer = (u8*)Ql_MEM_Alloc(sizeof(u8)*LWM2M_RECV_BUFFER_LENGTH);
