...
#define LWM2M_SEND_DATA_HEX

Enum_Lwm2m_Access_Mode lwm2m_access_mode =  LWM2M_ACCESS_MODE_BUFFER;
...
void proc_main_task(s32 taskId)
{
    ...
    APP_DEBUG("<--DaBai OpenCPU: LwM2M Client.-->\r\n");

    //register 
    ret = Ql_Timer_Register(LwM2M_TIMER_ID, Callback_Timer, NULL);
	APP_DEBUG("<--DaBai Ql_Timer_Register(%d)-->\r\n",ret);
		...
		{     
			switch (msg.param1)
            {
    		    ...
    		    case URC_LwM2M_RECV_DATA:
				{
					...
					{
						if(lwm2m_access_mode == LWM2M_ACCESS_MODE_DIRECT)
						{
						...
						{
							APP_DEBUG("<--lwm2m recv length(%d)-->\r\n",lwm2m_urc_param_ptr->recv_length);
							m_lwm2m_state = STATE_LwM2M_RD;
						}
    ...
    if (LwM2M_TIMER_ID == timerId)
    {
        ...
        {        
            ...
            case STATE_LwM2M_ADDOBJ:
            {
				...
				lwm2m_send_param_t.ins_id = 0;	 //Instance id
            ...
            {
				#ifdef LWM2M_SEND_DATA_HEX
				   ret = RIL_QLwM2M_Cfg(LWM2M_DATA_FORMAT_HEX,LWM2M_DATA_FORMAT_HEX);
			    #else
				   ret = RIL_QLwM2M_Cfg(LWM2M_DATA_FORMAT_TEXT,LWM2M_DATA_FORMAT_HEX);
				#endif 
                if (RIL_AT_SUCCESS == ret)
                {
