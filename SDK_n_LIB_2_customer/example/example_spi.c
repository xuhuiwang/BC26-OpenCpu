...
void CallBack_SPI_Hdlr(u8 * pRdBuffer,u32 rdLen)
{
    u32 i = 0;
    spi_flash_cs(1);
    APP_DEBUG("\r\n<-- SPI Interrupt Coming! Recv:%d\r\n",rdLen); 
    for(i = 0; i < rdLen;i++)
    {
       APP_DEBUG("%d ",*(pRdBuffer+i));   
    }
    APP_DEBUG("End of Recv!\r\n");
}
...
//@flash read data with callback
void flash_rd_data_ex(u32 addr, u32 len)
{
    s32 ret = 0;   
    u8 cmd = 0x03;
    u8 wr_buff[4]={cmd,(addr>>16)&0xff,(addr>>8)&0xff,addr & 0xff};
	...
	spi_flash_cs(0);
    ret = Ql_SPI_WriteRead_Ex(USR_SPI_CHANNAL,wr_buff,4,len,(SPI_CALLBACK *)CallBack_SPI_Hdlr);
}
    ...
    switch (msg)
    {
        case EVENT_UART_READY_TO_READ:
        {
            ...
            {        
                ...
                // read data from spi flash with callback.
                Ql_memset(buffer, 0x0, sizeof(buffer));
                Ql_sprintf(buffer, "ql_spi_ex_rd_flash_data");               
                ret = Ql_strncmp(Read_Buffer, buffer, Ql_strlen(buffer)); 
                if (!ret)
                {
                    //u8 buff[255]={0};
                    if (cmd_is_over())
                        flash_rd_data_ex(0,255);
                    else
                        APP_DEBUG("error.\r\n");
