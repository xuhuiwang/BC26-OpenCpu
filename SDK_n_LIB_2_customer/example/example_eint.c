 ...
 *   All the gpios can be configured to EINT mode.  
 *
 * Usage:
 * ------
 *   Compile & Run:
...
Enum_PinName pinname = PINNAME_RI;
/*****************************************************************
* callback function
******************************************************************/
...
void proc_main_task(s32 taskId)
{
    ...
    *                   Hardware debounce. 
    *               swDebounce:
    *                   Software debounce.Current only support hw debounce
    *		   automask:
    *	              mask the Eint after the interrupt happened.
    **************************************************************/    
    ret = Ql_EINT_Init(pinname, EINT_LEVEL_TRIGGERED, 200, 0,0);
    if(ret != 0)
    {
